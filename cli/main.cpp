...
ppmClientBudgetsStatus()
{
    ...
    const bool ok =
        copyUInt32Property(
            service,
            CFSTR("UseOverrideClientPowerBudgets"),
            &value);
    ...
    if (!ok) {
        std::fprintf(
            stderr,
            "could not read UseOverrideClientPowerBudgets\n");
        return 1;
    }

    std::printf(
        "UseOverrideClientPowerBudgets=%u\n",
        value);
...
static int
ppmSetClientBudgets(uint32_t value)
{
    if (value > 1) {
        std::fprintf(
            stderr,
            "client-budget override value must be 0 or 1\n");
        return 2;
    }
    ...
    std::printf(
        "UseOverrideClientPowerBudgets before=%u after=%u\n",
        before,
        after);
...
ppmCPMSIOReport(unsigned intervalMs)
{
    ...
    for (CFStringRef subgroup : subgroups) {
        CFDictionaryRef one =
            api.copyChannelsInGroup(
                CFSTR("PPM Stats"),
                subgroup,
    ...
    if (!channels) {
        std::fprintf(
            stderr,
            "no CPMS PPM Stats channels found\n");
        dlclose(api.handle);
        return 1;
    }
    ...
    if (!subscription) {
        std::fprintf(
            stderr,
            "CPMS IOReport subscription failed\n");
        if (subscribed)
            CFRelease(subscribed);
        CFRelease(channels);
        dlclose(api.handle);
    ...
    if (!first) {
        std::fprintf(
            stderr,
            "first CPMS IOReport sample failed\n");
        if (subscribed)
            CFRelease(subscribed);
        CFRelease(channels);
        dlclose(api.handle);
    ...
    if (!second) {
        std::fprintf(
            stderr,
            "second CPMS IOReport sample failed\n");
        CFRelease(first);
        if (subscribed)
            CFRelease(subscribed);
        CFRelease(channels);
    ...
    if (!arrayObject ||
        ...
        std::fprintf(
            stderr,
            "CPMS delta has no channels array\n");
        CFRelease(delta);
        if (subscribed)
            CFRelease(subscribed);
        CFRelease(channels);
